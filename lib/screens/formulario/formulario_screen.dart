...
class _FormularioScreenState extends State<FormularioScreen>
  ...
  TextEditingController _getController(String key) {
    return _controllers.putIfAbsent(
      key,
          () => TextEditingController(text: _formData[key] ?? ''),
    );
  }
    ...
    switch (_currentStep) {
      ...
      case 3:
        for (final bomba in widget.estacion.bombas) {
          ...
          if (encendido == null || encendido.isEmpty) {
            return 'Ingrese hora de encendido de ${bomba.nombre}';
          }
          if (apagado == null || apagado.isEmpty) {
          ...
          final esDiaSiguiente =
              _formData['bomba_${bomba.id}_apagado_siguiente_dia'] == '1';

          if (!esDiaSiguiente && minApa <= minEnc) {
    ...
    try {
      ...
      for (int turno = 1; turno <= 3; turno++) {
        ...
        if (nombre != null && nombre.trim().isNotEmpty) {
          operadores.add({'nombre': nombre.trim(), 'turno': turno});
        }
      }
      ...
      for (final bomba in widget.estacion.bombas) {
        ...
        if (encendido != null && apagado != null) {
          ...
          final esDiaSiguiente =
              _formData['bomba_${bomba.id}_apagado_siguiente_dia'] == '1';
          final fechaApagado =
          esDiaSiguiente ? hoy.add(const Duration(days: 1)) : hoy;

          bombeos.add({
            'bomba_id': bomba.id,
            'encendido': DateTime(hoy.year, hoy.month, hoy.day,
                int.parse(partsEnc[0]), int.parse(partsEnc[1]))
                .toIso8601String(),
            'apagado': DateTime(
                fechaApagado.year,
                fechaApagado.month,
                fechaApagado.day,
                int.parse(partsApa[0]),
                int.parse(partsApa[1]))
                .toIso8601String(),
            'horometro_inicial':
            horIni != null ? double.tryParse(horIni) : null,
            'horometro_final':
            horFin != null ? double.tryParse(horFin) : null,
          });
        }
      }
      ...
      final tableros = [
        {
          ...
          'estado': _formData['TABLERO_GENERAL_ESTADO'] ?? 'OK'
        },
        {
          'nombre': 'Tablero Bomba 1',
          'estado': _formData['TABLERO_BOMBA_1_ESTADO'] ?? 'OK'
        },
        {
          'nombre': 'Tablero Bomba 2',
          'estado': _formData['TABLERO_BOMBA_2_ESTADO'] ?? 'OK'
        },
      ];
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // Tabs de pasos con scroll horizontal
  ...
  // Campo de hora con diseño especial
